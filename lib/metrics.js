...
const metric = {
  ...
  node: {
    ...
    cpu: new client.Counter('nodejs_cpu_usage_microseconds_total', 'microseconds spent on CPU',['mode']),
    gc: {
      ...
      reclaimed: new client.Counter('nodejs_gc_reclaimed_bytes_total', 'total number of bytes reclaimed by GC',['gctype'])
    }
  }
}
...
function observeCPU(data) {
  metric.node.cpu.labels("user").inc(data.user - userCPU);
  userCPU = data.user
  metric.node.cpu.labels("system").inc(data.system - systemCPU);
  systemCPU = data.system
};
