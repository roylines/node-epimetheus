...
function observeCPU(data) {
  metric.node.cpu.labels("user").inc(data.user);
  metric.node.cpu.labels("system").inc(data.system);
};
...
function observeGC(stats) {
  ...
  if (stats.diff.usedHeapSize < 0) {
    totalReclaimed += stats.diff.usedHeapSize * -1;
    metric.node.gc.reclaimed.labels(gcTypes[stats.gctype]).inc(totalReclaimed);
  }
};
